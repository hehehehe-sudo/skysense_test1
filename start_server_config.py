...
class ServerManager:
    ...
    def start_all_workers(self) -> None:
        ...
        # ✅ 新增：启动完成后统一验证并打印状态表格
        self.verify_all_workers()

    def verify_all_workers(self) -> None:
        """Verify all registered workers' status and print a summary table"""
        self.logger.info("Verifying all registered workers...")
        all_configs = self.model_worker_config + self.tool_worker_config
        
        print("\n" + "=" * 70)
        print(f"{'Service Name':<20} | {'Worker Status':<15} | {'API/Local':<15} | {'Queue':<5} | {'Address'}")
        print("=" * 70)

        for cfg in all_configs:
            cfg = list(cfg.values())[0]
            ...
            try:
                ...
                worker_addr = resp.json().get("address", "").strip()
                if not worker_addr:
                    raise ValueError("Address not returned by controller")
                addr_str = worker_addr

                # 2. 探测 Worker 健康状态
                status_resp = requests.post(f"{worker_addr}/worker_get_status", json={}, timeout=10)
                status_resp.raise_for_status()
                status_data = status_resp.json()
                
                status_str = "🟢 Online"
                queue_str = str(status_data.get("queue_length", 0))

                # 3. 尝试调用 /worker_health 获取 API 连通性（若未添加该接口则降级判断）
                try:
                    health_resp = requests.post(f"{worker_addr}/worker_health", json={}, timeout=5)
                    if health_resp.status_code == 200:
                        h_data = health_resp.json()
                        if h_data.get("api_connected") is False:
                            type_str = "❌ API Down"
                        else:
                            type_str = "✅ API Ready" if "api_connected" in h_data else "✅ Local Model"
                    else:
                        type_str = "✅ Local Model"
                except:
                    type_str = "✅ Local Model"

            except Exception as e:
                self.logger.warning(f"Health check failed for {worker_name}: {e}")
                status_str = "🔴 Offline"
                type_str = "❌ Unreachable"
                queue_str = "-"
                addr_str = "-"

            print(f"{worker_name:<20} | {status_str:<15} | {type_str:<15} | {queue_str:<5} | {addr_str}")

        print("=" * 70 + "\n")
        self.logger.info("Worker verification complete.")
